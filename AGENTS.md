...
- Public runtime packages live under `brainbridge/run_lib` and `brainbridge/static_lib`.
...
- `brainbridge/run_lib/terminal_core` is the authoritative terminal input backend.
- Keep `terminial_core` only as a compatibility alias for existing callers.
...
- Sandbox archives under `.test/sandbox/` use `base/` for the baseline and `YYYY-MM-DD/` for dated snapshots.
...
- Preferred smoke tests:
  ...
  - `python3 -m brainbridge.static_lib.logger.log_core` when logging changes

## 中文

- 对外运行时包位于 `brainbridge/run_lib` 和 `brainbridge/static_lib`。
- 本地验证前先执行 `python3 -m pip install -e .`。
- 新代码直接从 `brainbridge...` 导入，不要再新增 `sys.path` 引导依赖。
- 旧的 bootstrap 辅助文件已经从现行源码树中移除。
- 不要把 `requests`、`chardet`、`pynput` 重新引回核心运行时依赖。
- `brainbridge/run_lib/terminal_core` 是权威终端输入后端。
- `terminial_core` 仅保留为兼容旧调用方的别名。
- `config/sys_conf` 是默认层，`config/user_conf` 会覆盖它。
- `storage/` 只是运行输出目录，不要把生成文件当源码提交，除非它们是刻意保留的 fixture。
- `.test/sandbox/` 里的 sandbox 归档规则是：`base/` 保存基线，`YYYY-MM-DD/` 保存日期快照。
- 如果修改了公共 API 或行为，记得同步两份 README 和两份 instruction 文档。
- 推荐烟雾测试：
  - `python3 .test/test_2.py`
  - 文件工具或 `.bb` 工具改动时跑 `python3 .test/test_1.py`
  - `.bb` 文件树头改动时跑 `python3 .test/test_7.py`
  - 终端交互改动时跑 `python3 .test/test_5.py`
  - 日志模块改动时跑 `python3 -m brainbridge.static_lib.logger.log_core`
