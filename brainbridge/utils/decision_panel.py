...
class DecisionPanelPage:
    def __init__(
        ...
        *,
        ...
        enable_input_box: bool = False,
        input_clear_keys: Optional[set[str]] = None,
        input_return_mode: Literal["selection", "dict"] = "selection",
        ...
        self._enable_input_box = bool(enable_input_box)
        self._input_clear_keys = {item.lower() for item in (input_clear_keys or set())}
        self._input_return_mode = input_return_mode
        self._highlight_current = bool(highlight_current)
    ...
    def _on_press(self, key: keyboard.Key | keyboard.KeyCode) -> bool:
        ...
        if self._enable_input_box and self._handle_input_key(key):
            self._draw()
            return True
    ...
    def _build_result(self) -> Any:
        selection = self._options[self._selected_index]["output"]
        if self._enable_input_box and self._input_return_mode == "dict":
            return {"selection": selection, "input": self._input_text}
        return selection

    def _handle_input_key(self, key: keyboard.Key | keyboard.KeyCode) -> bool:
        if self._matches_input_clear_key(key) and self._input_text:
            self._input_text = ""
            self._input_cursor = 0
            return True
        ...
        if key == keyboard.Key.backspace:
            if self._input_cursor > 0:
                self._input_text = (
                    self._input_text[:self._input_cursor - 1] + self._input_text[self._input_cursor:]
                )
        ...
        if key == keyboard.Key.delete:
            if self._input_cursor < len(self._input_text):
                self._input_text = (
                    self._input_text[:self._input_cursor] + self._input_text[self._input_cursor + 1:]
                )
                return True
            return False

        if key == keyboard.Key.home or key == keyboard.Key.ctrl_a:
            if self._input_cursor != 0:
                self._input_cursor = 0
                return True
            return False

        if key == keyboard.Key.end or key == keyboard.Key.ctrl_e:
            if self._input_cursor != len(self._input_text):
                self._input_cursor = len(self._input_text)
                return True
            return False

        if key == keyboard.Key.ctrl_u:
            if self._input_cursor > 0:
        ...
        if isinstance(key, keyboard.KeyCode):
            ...
            if not char or not char.isprintable():
                return False
            self._input_text = self._input_text[:self._input_cursor] + char + self._input_text[self._input_cursor:]
            self._input_cursor += len(char)
            return True

        return False

    def _matches_input_clear_key(self, key: keyboard.Key | keyboard.KeyCode) -> bool:
        if not self._input_clear_keys:
            return False
        if isinstance(key, keyboard.KeyCode):
            return key.char.lower() in self._input_clear_keys
        return key.value.lower() in self._input_clear_keys

    def _format_input_box_line(self, inner_w: int) -> str:
        prefix = "input: "
        cursor_text = self._input_text[:self._input_cursor] + "|" + self._input_text[self._input_cursor:]
        content = prefix + cursor_text
        if _display_width(content) > inner_w:
            suffix_width = max(1, inner_w - _display_width(prefix))
            content = prefix + _trim_to_width(cursor_text[-suffix_width:], suffix_width)
        return f"{self._theme.border_v}{_pad_to_width(content, inner_w)}{self._theme.border_v}"
    ...
    def _draw(self) -> None:
        ...
        # Do not draw into the last terminal column/row.
        # Many terminals auto-wrap when a printed line reaches the final column,
        # and printing exactly `rows` lines with trailing newlines can scroll the
        # panel. Leaving a one-cell margin keeps redraws stable in VSCode and
        # common Linux terminals.
        width = max(2, cols - 1)
        inner_w = max(10, width - 2)

        available_rows = max(1, rows - 1)
        header_rows = 1
        sep_rows = 1
        footer_rows = 4 if self._enable_input_box else 3
        ...
        lines.append(f"{t.border_top_left}{t.border_h * left_pad}{title}{t.border_h * right_pad}{t.border_top_right}")
        ...
        for i in range(start, end):
            ...
            if i == self._selected_index:
                ...
                line = f"   {label}   "
            rendered_line = _pad_to_width(line, inner_w)
            if i == self._selected_index and self._highlight_current:
                rendered_line = f"\033[7m{rendered_line}\033[0m"
            lines.append(f"{t.border_v}{rendered_line}{t.border_v}")

        for _ in range(usable_rows - (end - start)):
            lines.append(f"{t.border_v}{' ' * inner_w}{t.border_v}")

        lines.append(f"{t.border_sep_left}{t.border_h * inner_w}{t.border_sep_right}")
        ...
        if self._enable_input_box:
            lines.append(self._format_input_box_line(inner_w))
        lines.append(f"{t.border_v}{hint_1}{t.border_v}")
        lines.append(f"{t.border_v}{hint_2}{t.border_v}")
        lines.append(f"{t.border_bottom_left}{t.border_h * inner_w}{t.border_bottom_right}")
        sys.stdout.write("\r\n".join(lines) + "\r\n")
        sys.stdout.flush()
